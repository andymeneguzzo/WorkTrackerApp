...
public class ShiftAdapter extends RecyclerView.Adapter<ShiftViewHolder> {
    ...
    public void onBindViewHolder(ShiftViewHolder holder, int position) {
        ...
        holder.tvDate.setText("Data: " + shift.getDate());
        holder.tvTimeInterval.setText(shift.getStartTime() + " - " + shift.getEndTime());
        holder.tvHoursWorked.setText("Ore Lavorate: " + String.format(Locale.getDefault(), "%.2f", shift.getHoursWorked()));
        holder.tvHourlyWage.setText("Paga Oraria: " + String.format(Locale.getDefault(), "%.2f", shift.getHourlyWage()) + " €");
        holder.tvTotalPay.setText("Totale: " + String.format(Locale.getDefault(), "%.2f", shift.getTotalPay()) + " €");

        // Imposta il listener per l'intero elemento della lista
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                // Crea un Intent per avviare EditShiftActivity
                Intent intent = new Intent(context, EditShiftActivity.class);
                intent.putExtra("shift", shift); // Shift ora implementa Parcelable
                context.startActivity(intent);
            }
        });
    }
